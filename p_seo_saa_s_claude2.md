...
**Goal for MVP:** keep ops minimal and costs near $0 until traction; upgrade within Supabase as usage grows.

**Frontend/Renderer:** Next.js 14+ (App Router) with hybrid **SSG + ISR/SSR**. Tailwind for Template Studio preview.

**Backend API:** Next.js API routes (or a small Fastify/NestJS service if preferred later). GraphQL/REST for entities/templates.

**Database:** **Supabase Postgres** (managed). Use Prisma (or Supabase client) with RLS for multi‑tenancy. Enable **pgvector** in Supabase for duplication checks and clustering. Start on the **free tier**; upgrade plans when row counts/storage/throughput increase.

**Storage:** **Supabase Storage** for CSV uploads, images, and OG renders. Served through Supabase CDN. Begin on **free tier**; scale up as assets grow.

**Queues/Jobs:** Upstash Redis (or Supabase Functions/CRON for light jobs) for publish throttling, sitemap rebuilds, and QA checks.

**Observability:** Sentry (errors) + basic logs. Lighthouse CI on sample pages (optional) for Core Web Vitals.

**Multi‑tenancy:** Row‑level security in Supabase; per‑tenant schema via RLS policies and org_id scoping.

**Index Control:** Feature flags at tenant/template/page level. Publishing states are persisted; robots/meta can be toggled.

> **Why Supabase for DB & Storage?** Traffic is low initially, so Supabase’s **free plan** is sufficient. When usage increases (rows, bandwidth, or storage), simply upgrade within Supabase—no migration needed.
